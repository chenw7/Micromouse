...
/*
 * Wall in front: FL(1300), FR(2350)
 * Smth to left or right 1350
 ...
 */
  ...
  while (1)
  {
	  ...
	  IRLeft = readLeftIR();
	  IRRight = readRightIR();
	  IRFrontLeft = readFrontLeftIR();
	  IRFrontRight = readFrontRightIR();

	  if(IRFrontLeft < 1000 && IRFrontRight < 2000){
		  move(1);
	  }
	  else if(IRRight < 1000){
		  turn(1);
	  }
	  else{
		  turn(-1);
